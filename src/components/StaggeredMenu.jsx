            ...
            >
                ...
                <aside
                    ...
                    className={`staggered-menu-panel absolute top-0 right-0 h-full flex flex-col p-[2em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-[16px] pointer-events-auto transition-colors duration-500
                        ${theme === 'dark' ? 'bg-[#020806]/90 text-white' : 'bg-[#020806]/90 text-white'}`}
                    style={{ WebkitBackdropFilter: 'blur(16px)' }}
                    aria-hidden={!open}
                >
                    <div className="flex justify-end mb-4">
                        <button onClick={closeMenu} className="text-3xl text-white transition-colors hover:text-emerald-400">
                            &times;
                        </button>
                    </div>
                            ...
                            {items && items.length ? (
                                items.map((it, idx) => (
                                    <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                                        <a
                                            className={`sm-panel-item relative font-semibold text-[3rem] cursor-pointer leading-none tracking-normal uppercase transition-[all] duration-150 ease-linear inline-block no-underline pr-[1.4em]
                                                ${theme === 'dark' ? 'text-white hover:text-emerald-300' : 'text-white hover:text-emerald-300'}`}
                                            href={it.link}
                                            onClick={(e) => {
                                                e.preventDefault();
                                                if (it.onClick) {
                                ...
                                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                                    <span className="sm-panel-item relative font-semibold text-[3rem] cursor-pointer leading-none tracking-normal uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]">
                                        <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform">
                                            No items
                                        </span>
                                    </span>
                                        ...
                                        <li key={s.label + i} className="sm-socials-item">
                                            <a
                                                ...
                                                className={`sm-socials-link text-[1.2rem] font-medium no-underline relative inline-block py-[2px] transition-[color,opacity] duration-300 ease-linear
                                                    ${theme === 'dark' ? 'text-slate-300 hover:text-emerald-300' : 'text-slate-300 hover:text-emerald-300'}`}
                                            >
                                                {s.label}
                                            </a>
                                        </li>
...
.sm-scope .sm-panel-item { position: relative; font-weight: 600; font-size: 3rem; cursor: pointer; line-height: 1; letter-spacing: 0; text-transform: uppercase; transition: background 0.25s, color 0.25s; display: inline-block; text-decoration: none; padding-right: 1.4em; }
